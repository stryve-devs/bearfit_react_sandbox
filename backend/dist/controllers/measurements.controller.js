...
const createMeasurement = async (req, res) => {
    try {
        ...
        const { measurement_date, body_weight, waist, body_fat, lean_body_mass, neck, shoulder, chest, left_bicep, right_bicep, left_forearm, right_forearm, abdomen, left_thigh, right_thigh, left_calf, right_calf, entry_image_url, entry_image_key, } = req.body || {};
        if (!measurement_date)
        ...
        if (entry_image_key) {
            try {
                const parts = entry_image_key.split('.');
                ...
                const updated = await prismaClient_1.default.measurement.update({
