                    ...
                    <View style={styles.section}>
                        <BlurView intensity={28} tint="dark" style={styles.metricsContainer}>
                            ...
                            <TouchableOpacity
                                ...
                                onPress={() => setMuscleDistributionVisible(true)}
                            >
                                <Ionicons name="bar-chart" size={24} color={AppColors.orange} />
                            </TouchableOpacity>
                        </BlurView>
    ...
    return (
        <BlurView intensity={28} tint="dark" style={styles.exerciseCard}>
            ...
            {isWeightBased ? (
                ...
                <View style={styles.tableHeaderRow}>
                    <Text style={[styles.tableHeaderCell, { flex: 0.5 }]}>Set</Text>
                    <Text style={[styles.tableHeaderCell, { flex: 1.5, textAlign: 'center' }]}>Reps</Text>
                    <Text style={[styles.tableHeaderCell, { flex: 0.8 }]}>Done</Text>
                </View>
            )}
...
function MuscleDistributionSheet({ visible, exercises, totalSets, onClose }: any) {
    ...
    const sortedMuscles = Object.entries(muscleDistribution).length > 0
        ? Object.entries(muscleDistribution)
            ...
            .map(([muscle, data]) => ({ muscle, completed: data.completed, total: data.total }))
        : [];
    ...
    return (
        <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
            <TouchableOpacity
                ...
                onPress={onClose}
            >
                <BlurView intensity={25} tint="dark" style={styles.muscleDistributionSheet}>
                    <View style={styles.sheetHandle} />

                    <Text style={styles.muscleDistributionTitle}>Muscle Distribution</Text>

                    <View style={styles.muscleIconContainer}>
                        <Ionicons name="bar-chart" size={60} color={AppColors.orange} />
                    </View>
                    ...
                    >
                        {sortedMuscles.length === 0 ? (
                            <View style={styles.emptyMuscleContainer}>
                                <Text style={styles.emptyMuscleText}>No exercises added yet</Text>
                            </View>
                        ) : (
                            sortedMuscles.map((item, index) => {
                                const percentage = getProgressPercentage(item.completed, item.total);
                                return (
                                    <View key={`${item.muscle}-${index}`} style={styles.muscleItem}>
                                        <View style={styles.muscleRowHeader}>
                                            <Text style={styles.muscleRowName}>{item.muscle}</Text>
                                            <Text style={styles.muscleRowSetCount}>{item.completed}/{item.total}</Text>
                                        </View>
                    ...
                    <View style={styles.muscleDistributionFooter}>
                        <Text style={styles.totalCompletedText}>
